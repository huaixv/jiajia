...
void memprotect(caddr_t addr, size_t len, int prot) {
  ...
  if (protyes != 0) {
    sprintf(errstr, "mprotect failed! addr=0x%x, errno=%d", (unsigned long)addr,
            errno);
    assert(0, errstr);
  }
}
...
void memmap(caddr_t addr, size_t len, int prot) {
  ...
  if (mapad != addr) {
    sprintf(errstr, "mmap failed! addr=0x%x, errno=%d", (unsigned long)(addr),
            errno);
    assert(0, errstr);
  }
}
...
void memunmap(caddr_t addr, size_t len) {
  ...
  if (unmapyes != 0) {
    sprintf(errstr, "munmap failed! addr=0x%x, errno=%d", (unsigned long)addr,
            errno);
    assert(0, errstr);
  }
}
...
unsigned long jia_alloc3(int size, int block, int starthost) {
  ...
  mapsize =
      ((block % Pagesize) == 0) ? (block) : ((block / Pagesize + 1) * Pagesize);
  homepid = starthost;
...
unsigned long jia_alloc1(int size) { return (jia_alloc3(size, size, 0)); }
...
#if defined AIX41 || defined IRIX62
    void sigsegv_handler(int signo, int code, struct sigcontext *scp,
                         char *addr)
#endif
  ...
  */
  if ((H_MIG == ON) && (homehost(paddr) != jia_pid)) {
    ...
    assert((homehost(paddr) == jia_pid),
           "This should have not been happened! 4");
...
unsigned long jia_alloc(int size) { return (unsigned long)valloc(size); }
#endif /* NULL_LIB */
